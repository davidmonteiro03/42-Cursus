...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/10/26 14:57:04 by dcaetano          #+#    #+#             */
/*   Updated: 2023/10/26 15:23:50 by dcaetano         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	push(t_node **stack_from, t_node **stack_to)
{
	...
	if (stack_from == NULL || *stack_from == NULL)
		return (true);
	*stack_from = frstlastnode(*stack_from, false);
	*stack_to = frstlastnode(*stack_to, false);
	tmp = (*stack_from)->next;
	if (tmp != NULL)
		tmp->prev = NULL;
	...
	*stack = frstlastnode(*stack, false);
	...
	if (!reverse)
	{
		*stack = (*stack)->next;
		(*stack)->prev = NULL;
		tmp->next = NULL;
		tmp->prev = frstlastnode(*stack, true);
		frstlastnode(*stack, true)->next = tmp;
	}
	else
	{
		*stack = frstlastnode(*stack, true);
