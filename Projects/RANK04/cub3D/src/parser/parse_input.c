...
/*   Updated: 2023/12/10 14:26:01 by dcaetano         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (argc != 2)
		return (free(cub), cub_error(ft_strdup(ERROR_INPUT), false));
...
char	*cub_check_extension(t_cub *cub, char *arg, char *extension)
{
	...
	if (ft_strlen(new_arg) < extension_len + 1)
		return (free(cub), free(new_arg), \
			cub_error(ft_strdup(ERROR_LENGTH), false), NULL);
...
void	cub_check_input(t_cub *cub, int argc, char **argv)
{
	...
	cub->config.data.filename = cub_check_extension(cub, argv[1], ".cub");
}
