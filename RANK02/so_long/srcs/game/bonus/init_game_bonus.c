	...
	while (g->data.m[++i])
	{
		...
		while (g->data.m[i][++j])
		{
			if (g->data.m[i][j] == c.collect)
				put_image_bonus(g, g->img.std.col, j * 32, i * 32);
			else if (g->data.m[i][j] == c.start_pos)
				put_image_bonus(g, g->img.play.play, j * 32, i * 32);
			else if (g->data.m[i][j] == c.exit)
				put_image_bonus(g, g->img.std.ext, j * 32, i * 32);
			else if (g->data.m[i][j] == c.wall)
				put_image_bonus(g, g->img.std.wal, j * 32, i * 32);
			else if (g->data.m[i][j] == c.empty)
				put_image_bonus(g, g->img.std.bck, j * 32, i * 32);
		}
	}
}
